...
class DistributionTest(unittest.TestCase):
    ...
    def test_post_distribution(self):
        ...
        d_res, v_res, e_res, response = common_test_helper.post_distribution(
    ...
    def test_update_distribution(self):
        ...
        d_res, v_res, e_res, response_from_post = common_test_helper.post_distribution(
            common_test_helper.dataset_event,
            common_test_helper.version_event,
            common_test_helper.edition_event,
            common_test_helper.distribution_event,
        ...
        response_from_post = distribution_handler.update_distribution(
            distribution_event_for_put, None
        )
        response_from_distribution_put = common_test_helper.read_result_body(
            response_from_post
        )

        db_response = distribution_table.query(
            KeyConditionExpression=Key(table.DISTRIBUTION_ID).eq(
                response_from_distribution_put
            )
        )
    ...
    def test_get_all_distributions(self):
        ...
        response = distribution_handler.get_distributions(
            path_parameter_for_get_event, None
        )
    ...
    def test_get_one_distribution(self):
        ...
        path_parameter_for_get_event = {
            "pathParameters": {
                ...
                "version-id": common_test_helper.read_result_body(v_res),
                "edition-id": common_test_helper.read_result_body(e_res),
                "distribution-id": common_test_helper.read_result_body(
                    response_from_post
                ),
            }
        }

        get_all_response = distribution_handler.get_distribution(
            path_parameter_for_get_event, None
        )
        ...
        ] == common_test_helper.read_result_body(response_from_post)
